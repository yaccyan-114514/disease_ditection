...
<div class="header">
    <h1>编辑专家</h1>
    <a href="${pageContext.request.contextPath}/admin/experts/${expert.id}">返回详情</a>
</div>
<div class="container">
    <div class="card">
        <form method="post" action="${pageContext.request.contextPath}/admin/experts/${expert.id}/edit">
            <div class="field">
                <label>姓名</label>
            ...
            <div class="field">
                <label>手机号</label>
                <input type="text" name="phone" value="${expert.phone}" required>
            </div>
            <div class="field">
                <label>邮箱</label>
            ...
            <div class="field">
                <label>密码</label>
                <input type="text" name="password" value="${expert.password}" required>
            </div>
            <div class="field">
                <label>机构</label>
            ...
            <div class="field">
                <label>简介</label>
                <textarea name="bio">${expert.bio}</textarea>
            </div>
            <div class="field">
                <label>状态</label>
                <select name="status">
                    <option value="active" ${expert.status == 'active' ? 'selected' : ''}>正常</option>
                    <option value="disabled" ${expert.status == 'disabled' ? 'selected' : ''}>禁用</option>
                </select>
            </div>
            <div class="actions">
                <button type="submit" class="btn btn-submit">保存</button>
                <a href="${pageContext.request.contextPath}/admin/experts/${expert.id}" class="btn btn-cancel">取消</a>
            </div>
        </form>
    </div>
</div>
